...
// For Node.js environments, we need to set up the worker properly
let workerConfigured = false;
async function configurePdfJsWorker() {
    if (workerConfigured) return;
    try {
        // For Next.js server-side, get the absolute path to the worker and convert to file URL
        ...
        console.log("[CV Parser] Worker configured:", workerUrl);
    } catch (error) {
        console.error("[CV Parser] Failed to configure worker:", error);
        // Set to empty string as fallback (may not work but we'll handle the error)
    ...
    try {
        ...
        if (file.type === "application/pdf") {
            try {
                ...
                // Configure worker before using pdfjs-dist
...
//# sourceMappingURL=%5Broot-of-the-server%5D__7aae5e04._.js.map